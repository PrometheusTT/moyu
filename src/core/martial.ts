...
export type FormProgress = Record<SwordArt, [number, number, number]>;
export const freshFormProgress = (): FormProgress => Object.fromEntries(
  Object.keys(SWORD_ARTS).map(art => [art, [0, 0, 0]])) as FormProgress;
export function parseFormProgress(value: unknown): FormProgress {
  const result = freshFormProgress();
  if (!value || typeof value !== 'object') return result;
  for (const art of Object.keys(result) as SwordArt[]) {
    const row = (value as Record<string, unknown>)[art];
    if (Array.isArray(row) && row.length === 3 && row.every(n => Number.isSafeInteger(n) && n >= 0))
      result[art] = row.map(n => n % (SWORD_FORMS[art].length / 3)) as [number, number, number];
  }
  return result;
}
/** qi 是档位门槛，不是消费额；每档独立轮换，只有高档最后一式带收势。 */
export function selectSwordForm(art: SwordArt, qi: number, progress: readonly number[] = [0, 0, 0]):
  { index: number; tier: 0 | 1 | 2; full: boolean; cost: number } | null {
...
export const swordFormIndex = (cast: SwordCast): number => cast.formIndex ?? (cast.full ? SWORD_FORMS[cast.art].length - 1 : 0);
...
export const artRecovery = (full = false): number => full ? 0.9 : 0.42;
export type Cultivation = { insight: number; mastery: Record<SwordArt, number> };
export const GROWTH_INSIGHT = [0, 20, 60, 120, 220, 360, 540] as const;
/** 永久成长从已有阅历派生，旧存档不用迁移，也不保存可漂移的属性副本。 */
export function playerGrowth(insight: number): {
  level: number; maxHp: number; skillCooldown: number; armorDuration: number; armorCooldown: number; nextInsight: number | null;
} {
  const earned = Number.isFinite(insight) ? Math.max(0, insight) : 0;
  let tier = 0;
  while (tier + 1 < GROWTH_INSIGHT.length && earned >= GROWTH_INSIGHT[tier + 1]!) tier++;
