...
export class ChapterDirector {
  ...
  private encounterSpawned = false;
  ...
  chapterTitle(): string { return CHAPTER_TITLES[(this.chapter - 1) % CHAPTER_COUNT] ?? ''; }

  /** 当前章的收尾旁白（供 HUD 完成屏用）。 */
  chapterStory(): string { return CHAPTER_STORY[(this.chapter - 1) % CHAPTER_COUNT] ?? ''; }
  ...
  step(world: World, dt: number, input: Intent): boolean {
    ...
    if (this.activeStep >= PINCER_END && !this.encounterSpawned) {
      if (this.chapter % 9 === 0) this.encounterSpawned = world.spawnDuelist(this.side(this.chapter), this.chapter, true);
      else if (this.chapter % 3 === 0) this.encounterSpawned = world.spawnBoss(this.side(this.chapter), this.chapter);
      else if (this.chapter % 5 === 0) this.encounterSpawned = world.spawnDuelist(this.side(this.chapter), this.chapter);
      else this.encounterSpawned = true;
    }
    if (this.activeStep < CHAPTER_STEPS) this.schedule(world);
    ...
    if (this.activeStep === CHAPTER_STEPS && this.encounterSpawned && world.enemies.length === 0 && world.respawn <= 0
      && world.swordCast === null) this.finish(world);
    return true;
  }

  nextChapter(world: World): boolean {
    if (this.result === null || this.chapter >= Number.MAX_SAFE_INTEGER || world.phase !== 'fight') return false;
    this.chapter++;
    this.begin(world);
    return true;
  }
   ...
   */
  restartRun(world: World): boolean {
  ...
  private begin(world: World): void {
    world.biome = (this.chapter - 1) % CHAPTER_COUNT;
    world.beginChapter();
    if (this.saved) world.rng.restore(this.saved.rngState);
    this.activeStep = 0;
    this.result = null;
