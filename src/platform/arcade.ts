...
class InputLatch {
  private sequence: Array<{ key: string; at: number }> = [];
  ...
  private record(key: string, now: number): void {
    ...
    if ('WASD'.includes(key)) {
      if (this.sequence.at(-1)?.key !== key) this.sequence.push({ key, at: now });
    ...
    for (let i = 0; i < bytes.length; i++) {
      ...
      if (this.escape === 'seq') {
        if (b >= 0x40 && b <= 0x7e) {
          ...
          else if (b === 0x42) this.downUntil = this.hold(this.downUntil, now);
          else if (b === 0x43) { this.rightUntil = this.hold(this.rightUntil, now); this.lastHorizontal = 1; }
          else if (b === 0x44) { this.leftUntil = this.hold(this.leftUntil, now); this.lastHorizontal = -1; }
  ...
  clear(): void {
    this.sequence = []; this.pendingArt = undefined;
...
class StickGame implements GameInstance {
  ...
  combatHud(rows: number): string[] {
    ...
    if (rows <= 2) return [`${status} ${boss ? `王${boss.hp}` : `${this.director.chapter}关`}`,
