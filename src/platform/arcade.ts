  ...
  feed(bytes: Uint8Array, now: number): 'leave' | 'next' | 'help' | 'view' | 'play' | null {
    ...
    for (let i = 0; i < bytes.length; i++) {
      ...
      if (b === 0x1b && (bytes[i + 1] === 0x5b || bytes[i + 1] === 0x4f)) {
        ...
        else if (f === 0x43) this.rightUntil = now + 150;
        else if (f === 0x44) this.leftUntil = now + 150;
        i = j;
        continue;
      }
      if (b === 0x61 || b === 0x68) this.leftUntil = now + 180;
      else if (b === 0x64 || b === 0x6c) this.rightUntil = now + 180;
...
class SnakeGame implements GameInstance {
  ...
  update(dt: number, input: GameInput): void {
    if ((input.up || input.jump) && this.dir[1] !== 1) this.dir = [0, -1];
    else if (input.down && this.dir[1] !== -1) this.dir = [0, 1];
    else if (input.left && this.dir[0] !== 1) this.dir = [-1, 0];
    else if (input.right && this.dir[0] !== -1) this.dir = [1, 0];
    this.acc += dt;
  ...
  private reset(): void { this.body = [[8, 4], [7, 4], [6, 4], [5, 4], [4, 4]]; this.dir = [1, 0]; this.score = 0; }
...
export class Arcade {
  ...
  private instances: GameInstance[]; private canvases: LogicalCanvas[]; private microCanvases: LogicalCanvas[];
  ...
  private expandedCanvases: LogicalCanvas[];
  ...
  constructor(eventsFile?: string, modules: GameModule[] = BUILTIN_GAMES, startId?: string) {
    const context: GameContext = { seed: Date.now() & 0x7fffffff, random: () => Math.random() };
  ...
  feed(bytes: Uint8Array, now = Date.now()): boolean {
    ...
    if (command === 'next') {
      this.instances[this.active]!.onHostEvent?.('pause'); this.save(this.active);
      this.active = (this.active + 1) % this.instances.length; this.resetClock(); this.instructions = true;
    }
  ...
  panel(): [string, string] {
    ...
    if (!this.instructions) return [this.instances[this.active]!.hud?.() ?? m.name, 'E 大小 · ? 帮助 · Esc 返回'];
  ...
  advance(now: number): void {
    ...
    if (++this.poll >= 8) {
      ...
      for (const event of this.tail.poll()) {
        ...
        for (const game of this.instances) game.onHostEvent?.(e);
  ...
  render(target: PixelTarget, profile: 'standard' | 'micro' = 'standard'): void {
    ...
    if (profile === 'standard' && target.tier !== 'graphics' && game.renderExpanded !== undefined) {
      ...
      if (this.expandedCanvases[this.active]!.height !== height) this.expandedCanvases[this.active] = new LogicalCanvas(80, height);
      const canvas = this.expandedCanvases[this.active]!; game.renderExpanded(canvas); canvas.blit(target); return;
    }
  ...
  private stateFile(index: number): string { return path.join(process.env.MOYU_HOME ?? path.join(homedir(), '.moyu'), 'state', `${this.modules[index]!.manifest.id}.json`); }
  pause(): void { this.paused = true; this.resetClock(); this.save(this.active); this.instances[this.active]!.onHostEvent?.('pause'); }
  resume(): void { this.paused = false; this.resetClock(); this.instructions = true; this.urgent = false; this.notice = null; this.instances[this.active]!.onHostEvent?.('resume'); }
  private loadAll(): void {
    for (let i = 0; i < this.instances.length; i++) try {
      const value: unknown = JSON.parse(fs.readFileSync(this.stateFile(i), 'utf8')); this.instances[i]!.restore?.(value);
    } catch { /* first run or damaged save: start clean */ }
  }
  private save(index: number): void {
    const value = this.instances[index]!.serialize?.(); if (value === undefined) return;
    try { const file = this.stateFile(index); fs.mkdirSync(path.dirname(file), { recursive: true }); fs.writeFileSync(file, `${JSON.stringify(value)}\n`, { mode: 0o600 }); } catch { /* play must not depend on persistence */ }
  }
}
