...
function frame(head: Point, limbs: readonly (readonly Point[])[], sword: readonly Point[]): string[] {
  const grid = Array.from({ length: 8 }, () => Array<string>(20).fill('.'));
  const dot = (x: number, y: number, mark: string): void => { if (x >= 0 && x < 20 && y >= 0 && y < 8) grid[y]![x] = mark; };
  const line = (a: Point, b: Point, mark: string): void => {
    const steps = Math.max(Math.abs(b[0] - a[0]), Math.abs(b[1] - a[1]), 1);
    for (let i = 0; i <= steps; i++) dot(Math.round(a[0] + (b[0] - a[0]) * i / steps), Math.round(a[1] + (b[1] - a[1]) * i / steps), mark);
  };
...
export function drawMicroFighter(c: GameCanvas, f: Fighter, centerX: number, body: number, blade: number, lift = 0): void {
  ...
  for (let y = 0; y < sprite.length; y++) for (let x = 0; x < sprite[y]!.length; x++) {
    ...
    if (mark === '#' || (mark === '*' && f.armed)) c.pixel(pivot + (x - 8) * f.face, y + dy, mark === '#' ? body : blade);
  }
}
