...
export async function cmdPlay(id?: string): Promise<number> {
  ...
  if (id !== undefined && !modules.some((m) => m.manifest.id === id)) { process.stderr.write(`找不到游戏 ${id}\n`); return 2; }
  const arcade = new Arcade(undefined, modules, id);
  const teardown = new Teardown(() => ({}));
  let expanded = true;
  const rows = (): number => expanded ? Math.min(6, Math.max(2, (process.stdout.rows ?? 24) - 2)) : 2;
  ...
  let done = false, timer: NodeJS.Timeout | null = null;
  const layout = (): void => {
  ...
  return new Promise<number>((resolve) => {
    const finish = (): void => {
      ...
      teardown.run(); resolve(0);
    };
    teardown.install();
    teardown.onRestore(() => { if (timer !== null) clearInterval(timer); });
    teardown.onRestore(() => { try { process.stdin.setRawMode(false); } catch { /* terminal disappeared */ } });
    ...
    process.stdin.on('data', (b: Buffer) => {
      ...
      if (b.includes(0x03) || f12 || oneKeyClose || arcade.feed(b)) finish();
      else if (arcade.takeViewToggle()) { expanded = !expanded; layout(); }
    });
    process.stdout.on('resize', layout);
    timer = setInterval(() => {
      if (done) return;
      arcade.setDisplay(rows(), 'braille'); arcade.advance(Date.now());
      const h = arcade.hud();
