 ...
 * ## 显式隐藏为什么**不在**这个列表里
 *
 * 用户隐藏游戏时仍保留一行受保护区域，但里面只有倒数第二列的低对比度状态点。快捷键发现
 * 留在首次帮助和文档，不在候场暴露品牌或操作文案。隐藏由 `Shell.setCollapsed` 处理：停止
 * 游戏帧、归还输入、删除图片并保留内层滚动区。真正整屏让出的路径只留给终端太小——
 * 那时候连这一行都腾不出来，窗口恢复后只重建候场，不会自动重新取得游戏焦点。
 *
