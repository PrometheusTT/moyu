 ...
 *
 * 让屏 = 撤滚动区 + PTY 调成整屏 + 游戏彻底消失。现在只有一个原因会走到这里：
 *
 *   * `too-small` —— 终端小到塞不下两个区域。
 *
 * 用"原因集合"而不是布尔量：多个原因可以同时成立，而布尔量会在其中一个消失时错误地恢复。
 * 集合天然处理这个，所以这个结构留着 —— 下一个让屏原因进来时不用重写一遍翻转逻辑。
 *
 * ## `^G h` 为什么**不在**这个列表里（用户报的 bug 逼出来的）
 *
 * `^G h` 原来也是一个让屏原因（`user-hidden`）。用户报"按 h 收起以后没法打开了" ——
 * 而按键路径是好的（PTY-in-PTY 里再按一次确实会重新分屏、重新出帧）。坏的是**屏幕上
 * 再没有任何东西告诉你怎么回去**：整条连那行 `^G h 收起` 的提示一起消失了，剩下的
 * 唯一线索存在用户的记忆里。而他记得的那半个是 `h`，按下去只会进内层 CLI 的输入框。
 *
 * 所以收起改成"塌成 1 行"（`Shell.setCollapsed`）：那一行是我们的，写着 `^G h 展开`，
 * 40 行的终端里花 1 行换一条永远看得见的回头路。让屏那条路径只留给"终端太小" ——
 * 那时候连 1 行都腾不出来，用户也不需要提示（把窗口拉大就回来了）。
 *
...
export type YieldReason = 'too-small';
