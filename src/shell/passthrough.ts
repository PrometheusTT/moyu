...
import { DEFAULT_CELL } from '../render/caps.ts';

/** 内层可用的真实屏幕行范围（1-based，含端点）。 */
export type Region = { top: number; bottom: number };

export type PassthroughOptions = {
  region: Region;
  /** 内层可用的列数。分屏只切行不切列，所以它就是终端列数。 */
  cols: number;
  /** 一个字符格的设备像素（探测来的）。回答 `CSI 14/15/16 t` 要用。 */
  cell?: { w: number; h: number };
  /**
   * 内层查询尺寸时回调，参数是该**回给内层 stdin** 的字节（不是给终端的）。
   *
   * 不给这个回调的话尺寸查询就原样透传、由终端回答（= 改造之前的行为）。这是刻意的：
   * 吞掉一条查询却没人回答，内层可能就在那儿等着了。
   */
  ...
  /**
   * 内层发了 `ED 2` / `ED 3`（全屏擦除）时回调。
   *
   * 这个通知是**正确性要求**，不是优化：ED 不受滚动区约束，它会把游戏区一起擦掉，
...
export class Passthrough {
  ...
  private onFullClear: (() => void) | undefined;
  ...
  push(chunk: Uint8Array): Uint8Array {
    ...
    const flushCopy = (end: number): void => {
      if (copyFrom >= 0 && end > copyFrom) {
        for (let k = copyFrom; k < end; k++) out.push(chunk[k]!);
      }
      copyFrom = -1;
    };
  ...
  private step(b: number, out: number[]): void {
    switch (this.state) {
      case G_ESC: {
        this.pending.push(b);
        if (b === 0x5b) { this.state = G_CSI; return; }             // '['
        ...
        if (b >= 0x20 && b <= 0x2f) { this.state = G_ESC_INTER; return; }
      ...
      case G_ESC_INTER: {
        this.pending.push(b);
        if (b >= 0x20 && b <= 0x2f) return;                          // 还有中间字节
        this.state = G_GROUND;
        this.emitPending(out);
        return;
      }
      ...
      case G_STRING: {
        if (b === 0x07) { out.push(b); this.state = G_GROUND; return; }     // BEL 终结
  ...
  private emitPending(out: number[]): void {
    for (const x of this.pending) out.push(x);
    this.pending = [];
  }

  private static readonly encoder = new TextEncoder();

  private emitStr(out: number[], s: string): void {
  ...
  private emitCsi(out: number[]): void {
    ...
    const paramStart = i;
    while (i < seq.length - 1 && ((seq[i]! >= 0x30 && seq[i]! <= 0x39) || seq[i]! === 0x3b || seq[i]! === 0x3a)) i++;
    const paramEnd = i;
    let inter = '';
    while (i < seq.length - 1) { inter += String.fromCharCode(seq[i]!); i++; }

    // 有中间字节的一律不碰（DECSCUSR / DECRQM / DECSET-with-inter 等）
    if (inter !== '') { this.emitPending(out); return; }
    ...
    if (final === 0x4a) { // 'J'
      ...
      if (mode === 2 || mode === 3) this.onFullClear?.();
      this.emitPending(out);
      return;
    }
    ...
    if (final === 0x48 || final === 0x66 || final === 0x64) {
      ...
      if (final === 0x64) { // VPA: CSI r d
        ...
        if (r >= 1 && r <= span) { this.emitPending(out); return; } // 本来就在界内
      ...
      if (r >= 1 && r <= span) { this.emitPending(out); return; }
   ...
   * 因为内层能用的就那么大。
   */
  private sizeReply(kind: number | undefined): string {
  ...
  private parseParams(seq: readonly number[], start: number, end: number): number[] {
    const params: number[] = [];
    let cur = -1;
    for (let k = start; k < end; k++) {
      const b = seq[k]!;
      if (b >= 0x30 && b <= 0x39) cur = (cur < 0 ? 0 : cur) * 10 + (b - 0x30);
      else { params.push(cur); cur = -1; }
    }
    params.push(cur);
    return params;
  }
