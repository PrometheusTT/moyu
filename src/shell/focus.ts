...
export type Action =
  ...
  | { kind: 'toggle-focus' }
  | { kind: 'toggle-hidden' }
  | { kind: 'adjust-split'; delta: number }
  | { kind: 'redraw' }
  | { kind: 'quit' };
 ...
 */
function command(b: number): Action | 'unknown' {
  switch (b) {
...
/** kitty 的 `CSI code[:alt] ; mods[:event] ; text u` 里我们用得上的那几项。 */
type KittyKey = {
  /** unicode-key-code。ctrl+g 报的是 103（'g' 的码位），不是 0x07。 */
  code: number;
  ctrl: boolean;
  alt: boolean;
  /** 1=按下 2=自动重复 3=松开。缺省 1。 */
  event: number;
};

/** 修饰键自己的 code 区间（左右 shift/ctrl/alt/super/hyper/meta/iso）。 */
function isModifierOnly(code: number): boolean {
  return code >= 57441 && code <= 57452;
}

/** 第 `i` 个分号段的第 `sub` 个冒号子参数。空/缺省返回 undefined。 */
function subParam(params: string, i: number, sub: number): number | undefined {
 ...
 */
function ctrlByte(key: KittyKey): number | null {
  if (!key.ctrl || key.alt) return null;
  if (key.code < 0x40 || key.code > 0x7f) return null;
  return key.code & 0x1f;
}

type Scan =
  /** 完整一条序列，`end` 是结束位置（不含）。`key` 非空 = 它是一条 kitty 按键上报。 */
  | { end: number; key: KittyKey | null }
  /** 序列在 chunk 末尾被切断了，调用方该把这一段扣住等下一个 chunk。 */
  | 'partial'
  /** 这个位置不该当作一条转义序列，按普通字节处理。 */
  | null;
...
function scanEsc(buf: Uint8Array, at: number): Scan {
  ...
  if (next !== 0x5b) {
    ...
    if (next === 0x4f && buf[at + 2] !== undefined) return { end: at + 3, key: null };
    return { end: at + 2, key: null };
  }
  let j = at + 2;
  while (j < buf.length && buf[j]! >= 0x30 && buf[j]! <= 0x3f) j++;    // 参数字节（含私有前缀）
  const paramEnd = j;
  while (j < buf.length && buf[j]! >= 0x20 && buf[j]! <= 0x2f) j++;    // 中间字节
  if (j >= buf.length) return j - at <= MAX_HOLD ? 'partial' : null;
  const end = j + 1;
  if (buf[j] !== 0x75) return { end, key: null };                      // 不是 `u` 收尾，不是我们的事
  let params = '';
  for (let k = at + 2; k < paramEnd; k++) params += String.fromCharCode(buf[k]!);
  // `CSI > … u` / `CSI < u` / `CSI ? u` 是发给**终端**的标志操作，不是按键上报；
  // 终端对查询的回复（`CSI ? flags u`）也长这样，得原样交回给发起查询的内层。
  if (/[<=>?]/.test(params)) return { end, key: null };
...
export class InputRouter {
      ...
      if (b === 0x1b) {
        ...
        if (s !== null) {
          ...
          if (act === 'pass') {
            ...
            flushRun(i);
            if (act === 'inner') actions.push({ kind: 'forward', bytes: buf.subarray(i, s.end) });
            else if (act !== 'drop') actions.push(act);
          }
          i = s.end;
          continue;
        }
   ...
   * - Action：直接执行
   */
  private escSeq(key: KittyKey | null): Action | 'pass' | 'drop' | 'inner' {
    if (this.awaiting) {
      // 等命令状态下，一整条序列算**一个**键。修饰键自己和重复/松开事件不算键 ——
      // 它们不能消费掉等命令状态（按住 Shift 去按 '+' 时修饰键先到，一消费掉命令就丢了）。
      if (key !== null && (isModifierOnly(key.code) || key.event !== 1)) return 'drop';
      this.awaiting = false;
      if (key === null) return 'drop';       // 不认识的序列 = 按错了，整条丢掉（不能只丢 ESC）
    ...
    if (ctl === PREFIX) {
      ...
      if (key.event === 1) this.awaiting = true;
      return 'drop';
    }
    if (ctl === 0x03) return 'inner';
