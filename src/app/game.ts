...
import { World, NO_INTENT, type Intent } from '../core/world.ts';
...
export const displayTitle = (): string => isEnglish() ? 'Stick Slash: take a break, cut through the chaos' : TITLE;

export class Game {
  readonly world: World;
  readonly keys = new Keys();
  private readonly tail: SignalTail | null;
  ...
  constructor(opts: { seed?: number; events?: boolean; eventsFile?: string } = {}) {
  ...
  feed(bytes: Uint8Array): boolean {
    ...
    for (const cmd of this.keys.feed(bytes, Date.now())) {
      switch (cmd) {
        ...
        case 'task-done': this.signalDone('手动'); break;
        case 'task-start': this.world.taskStart(); break;
  ...
  advance(nowMs: number): void {
    if (this.lastMs === 0) { this.lastMs = nowMs; return; }
    ...
    if (this.tail !== null && ++this.pollN >= POLL_EVERY) {
      ...
      for (const sig of this.tail.poll()) {
        if (sig === 'start') this.world.taskStart();
        else this.signalDone(sig === 'notify' ? '要你确认' : '任务完成');
      }
    }
  ...
  private signalDone(why: string): void {
    if (this.world.phase === 'clear' || this.world.phase === 'paused') return;
