...
import { isEnglish, type GameLanguage } from '../i18n.ts';
...
function launchCommand(argv: string[]): string[] {
...
export function parseLanguageArgs(input: string[]): { argv: string[]; language?: GameLanguage; error?: string } {
  const separator = input.indexOf('--');
  const limit = separator < 0 ? input.length : separator;
  const argv: string[] = [];
  let language: GameLanguage | undefined;
  for (let i = 0; i < limit; i++) {
    const arg = input[i]!;
    if (arg === '--lang' || arg.startsWith('--lang=')) {
      const value = arg === '--lang' ? input[++i] : arg.slice('--lang='.length);
      if (value !== 'en' && value !== 'zh') return { argv: input, error: '--lang accepts only en or zh' };
      language = value;
    } else argv.push(arg);
  }
...
async function main(): Promise<number> {
  ...
  if (parsed.error !== undefined) { process.stderr.write(`moyu: ${parsed.error}\n`); return 2; }
  if (parsed.language !== undefined) process.env.MOYU_LANG = parsed.language;
  ...
  if (cmd === 'bench') {
    ...
    if (tier === 'bad') { process.stderr.write(tr('--tier= 只认 graphics、braille 或 half\n', '--tier= accepts only graphics, braille, or half\n')); return 2; }
...
function cmdInstall(args: string[]): number {
  ...
  if (targets.length === 0) {
    process.stderr.write(
      tr('没找到 Claude Code 或 Codex（~/.claude 和 ~/.codex 都不存在，PATH 上也没有）。\n',
  ...
  const out: string[] = [un ? tr('摸鱼 hook 卸载', 'Moyu hook removal') : tr('摸鱼 hook 安装', 'Moyu hook installation'), ''];
  let failed = false;
  ...
  for (const p of plans) {
    ...
    if (write) {
      ...
      const what = p.action === 'unchanged' ? tr('不用改（已经是想要的样子）', 'No change needed')
        ...
        : tr(`会更新${p.removed > 0 ? `（先摘掉 ${p.removed} 条旧的）` : ''}`,
          `Will update${p.removed > 0 ? ` (removing ${p.removed} old hooks first)` : ''}`);
    ...
    if (p.target === 'codex' && !un && p.action !== 'unchanged') {
      out.push(tr('  ! Codex 下次启动会问 "Hooks need review" —— 选 "Trust all and continue"，不然 hook 不会跑',
        '  ! On the next Codex launch, choose "Trust all and continue" when asked to review hooks.'));
    }
    out.push('');
  }

  out.push(`${tr('事件文件', 'Event file')}: ${homeVar(file)}`);
  if (!write) out.push('', tr('以上是干跑，什么都没改。真要写：', 'Preview only; nothing was changed. To apply: ') + (un ? 'moyu install --uninstall --write' : 'moyu install --write'));
  else if (!un && !failed) out.push(tr('可以了：moyu -- claude（或 moyu -- codex）。不装 hook 也能玩：游戏里按 t / y 手动喂信号。',
    'Ready: moyu -- claude (or moyu -- codex). You can also play without hooks and send signals with t / y.'));
...
function lastSignal(file: string): string {
  ...
  try {
    ...
    if (last === '') return tr('空的（还没有任何事件）', 'Empty (no events yet)');
    const parts = last.split(/\s+/);
    const kind = parts[parts.length - 1] ?? '?';
    const ts = Number(parts[0]);
    if (!Number.isFinite(ts) || parts.length < 2) return tr(`最后一条 ${kind}`, `Last event: ${kind}`);
...
async function cmdDoctor(): Promise<number> {
  ...
  let pty: string;
  try { await import('@lydell/node-pty'); pty = tr('可用', 'Available'); } catch (e) {
    const reason = e instanceof Error ? e.message.split('\n')[0] : String(e);
    pty = tr(`加载失败（${reason}）—— moyu -- <cmd> 用不了，moyu demo 照样能玩`,
      `Failed to load (${reason}); moyu -- <cmd> is unavailable, but moyu demo still works`);
  }
  out.push(`PTY         ${pty}`);
  ...
  for (const t of BOTH) {
    ...
    if (st.broken !== null) { out.push(`${name} ${homeVar(st.path)} ${tr('不是合法 JSON', 'contains invalid JSON')}: ${st.broken}`); continue; }
    ...
    if (t === 'codex') out.push(tr('            （Codex 那边还要在启动时点过一次 "Trust all and continue" 才真的会跑）',
      '            (Choose "Trust all and continue" on Codex launch to enable hooks)'));
  }

  out.push('', tr('渲染档位：moyu doctor --caps（要在真终端里跑）；只验证 Kitty 图片链路：moyu doctor --gfx',
    'Rendering tier: moyu doctor --caps (run in a real terminal); Kitty Graphics test: moyu doctor --gfx'),
    tr('终端被搞坏了：moyu doctor --reset', 'Restore terminal state: moyu doctor --reset'));
  process.stdout.write(`${out.join('\n')}\n`);
  return 0;
}
...
function englishCapsReason(caps: Awaited<ReturnType<typeof probeCaps>>, tty: boolean, mux: string | null,
  known: string | null): string {
  if (caps.why.startsWith('MOYU_TIER=')) return caps.why;
  if (process.env.MOYU_FORCE_GRAPHICS === '1') return `MOYU_FORCE_GRAPHICS=1, ${caps.cellW}×${caps.cellH} pixels/cell`;
  if (!tty) return 'Not a TTY';
  if (mux !== null) return `${mux} does not pass APC through; using Braille`;
  if (caps.probe === 'no-graphics') return 'Terminal answered DA but not Kitty Graphics; using Braille';
