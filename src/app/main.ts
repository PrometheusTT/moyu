...
import { computeLayout, adjustGameRows, scrollRegionSeq, fullScrollRegionSeq, assertLayout, fieldColsFor, DEFAULT_GAME_ROWS, type Focus, type Layout } from '../shell/regions.ts';
...
import { InputRouter, hotkeyHint, pendingRows } from '../shell/focus.ts';
import { Canvas } from '../render/canvas.ts';
import { GraphicsTarget } from '../render/graphics.ts';
import { probeCaps, DEFAULT_CELL } from '../render/caps.ts';
...
async function main(): Promise<number> {
  ...
  if (cmd === 'bench') {
    ...
    if (tier === 'bad') { process.stderr.write('--tier= 只认 graphics 或 half\n'); return 2; }
    return cmdBench(Number(argv[1] ?? 300), argv.includes('--stress'), argv.includes('--game'), argv.includes('--strip'), tier);
  }
  if (cmd === 'play') { process.stderr.write('moyu play（全屏游戏）是 M1 的内容，还没实现。\n'); return 1; }
...
function cmdBench(frames: number, stress: boolean, game: boolean, strip: boolean, tier: Tier): number {
  ...
  const t: PixelTarget = tier === 'graphics'
...
async function cmdCaps(): Promise<number> {
  ...
  const target: PixelTarget = caps.tier === 'graphics'
...
class Shell {
  ...
  private onScreenSwap(): void {
    setImmediate(() => {
      const l = this.layout;
      if (this.finished || l === null || this.arbiter.yielded) return;
  ...
  private tick(): void {
    ...
    if (body === '' && hud === this.lastHud && alert === null) return;
    this.lastHud = hud;
   ...
   */
  private hudSeq(l: Layout): string {
  ...
  private statusRows(h: { short: string; urgent: boolean }): string[] {
  ...
  private toggleFocus(): void {
    ...
    if (now - this.lastToggle < FOCUS_DEBOUNCE_MS) return;
    this.lastToggle = now;
