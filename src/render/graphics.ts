 ...
 *
 * 完整 1,800 步章节夹具（暗/浅色、普通/减少动态）测到：320×34 平均 1.48~1.54 KB、
 * 峰值约 2.61 KB；640×68 平均 3.29~3.50 KB、峰值 6.05~6.19 KB。后者最长会拆成
 * 两段 APC，但宿主仍把一幅完整图片放进同一次 `write`，不会让光标恢复字节插进帧中。
 * 这些数是持续移动、阵型、污迹、碎片和结算画面共同形成的参考基线，不是带宽承诺。
 *
 * 顺带解掉一条美术约束：字符档的背景**只能是水平色带**（加一朵云会让每帧全脏）；
 * 这一档的成本主要由压缩后的整幅 RGB 决定，而不是变了多少终端格，复杂背景仍须由 QA
 * 的平均/峰值载荷和编码分位数约束，不能假定免费。
 *
 * ## 几个不能改的取舍
 *
 ...
 * - **一帧一次宿主 `write`**：内容可以按协议拆成一段或多段 APC，但整帧必须一起写完，
 *   不能和光标还原的字节交错。
 */
...
export class GraphicsTarget implements PixelTarget {
  ...
  private cur32 = new Uint32Array(0);
  private prev32 = new Uint32Array(0);
  private dirtyAll = true;
  /** 终端里现在存着我们的图吗（决定 `disposeSeq` 要不要发）。 */
  private live = false;

  constructor(cols: number, rows: number, cellW: number, cellH: number) {
  ...
  private alloc(): void {
    ...
    for (let f = 1; f <= 8; f++) {
      ...
      if (this.cols * cw * this.rows * ch <= MAX_PIXELS) break;
    }
  ...
  setPixel(x: number, y: number, color: number): void {
    if (x < 0 || x >= this.pw || y < 0 || y >= this.ph) return;
  ...
  getPixel(x: number, y: number): number {
    if (x < 0 || x >= this.pw || y < 0 || y >= this.ph) return 0;
    const i = (y * this.pw + x) * 3;
  ...
  encode(screenTop: number, screenLeft = 1): string {
    ...
    let out = screenTop > 0 ? `${ESC}[${screenTop};${screenLeft}H` : '';
    if (b64.length <= CHUNK) {
      out += `${APC}${keys};${b64}${ST}`;
    } else {
 ...
 */
export function deleteImageSeq(): string {
