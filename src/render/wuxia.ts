import type { SwordCast } from '../core/martial.ts';
import { SWORD_ARTS, SWORD_FORMS, currentSwordForm, artDuration } from '../core/martial.ts';
import type { SceneTheme } from './theme.ts';
import { bossPhase, bossQuakeOffsets, bossArmored, bossAbilities, variantBossReach, duelistFormFor, type World } from '../core/world.ts';
import { fighterSegments } from '../core/creature.ts';

/** 世界坐标画笔，让 braille、半块和原生像素共用地标与剑招构图。 */
export type ArtPen = {
...
export function paintLandmark(p: ArtPen, width: number, ground: number, scene: SceneTheme): void {
  if (!scene.landmark) return;
  // 按固定纵横比续接景物，不把同一座山门/竹林拉成整幅超宽背景。
  const tile = ground * 4.5;
  if (tile <= 0 || !Number.isFinite(width)) return;
...
function paintLandmarkTile(p: ArtPen, width: number, ground: number, scene: SceneTheme): void {
  const far = scene.skyTint, near = scene.groundTint, light = scene.foeTint;
  const line = (a: number, b: number, c: number, d: number, color = near): void =>
    p.line(a * width, b * ground, c * width, d * ground, color);
  ...
  switch (scene.landmark) {
    ...
    case 'bridge':
      for (let i = 0; i < 16; i++) {
        ...
        const y = (x: number): number => 0.58 + (x - 0.5) ** 2 * 2;
        line(a, y(a), b, y(b));
        line(a, y(a) + 0.09, b, y(b) + 0.09);
        if (i % 2 === 0) line(a, y(a), a, y(a) - 0.16, far);
      }
    ...
    case 'pagoda': {
      ...
      for (let i = 0; i < tiers; i++) {
        ...
        for (const sign of [-1, 1]) line(0.5 + sign * half * 0.65, y + 0.06, 0.5 + sign * half * 0.65, 0.94);
      }
      break;
    }
    case 'desert':
...
export function paintSwordArt(p: ArtPen, cast: SwordCast | null, h: number): void {
  if (!cast || cast.age < 0 || cast.age >= artDuration(cast.art, cast.full)) return;
  ...
  const shade = (ink: number, brightness: number): number => {
    const bg = 0x171c23;
    const mix = (shift: number): number => Math.round(((bg >> shift) & 255) * (1 - brightness)
      + ((ink >> shift) & 255) * brightness);
    return (mix(16) << 16) | (mix(8) << 8) | mix(0);
  };
  type Point = [number, number];
  const line = (a: Point, b: Point, ink: number): void =>
    p.line(x + face * a[0], y + a[1], x + face * b[0], y + b[1], ink);
  const fill = (points: Point[], ink: number): void => {
    ...
    for (let yy = lo + step / 2; yy < hi; yy += step) {
      ...
      cuts.sort((a, b) => a - b);
      for (let i = 0; i + 1 < cuts.length; i += 2) line([cuts[i]!, yy], [cuts[i + 1]!, yy], ink);
    }
  };
  const blade = (a: Point, b: Point, width: number, ink: number, core = white): void => {
    const dx = b[0] - a[0], dy = b[1] - a[1], len = Math.hypot(dx, dy);
    if (len < h * 0.04 || width < h * 0.008) return;
    const nx = -dy / len, ny = dx / len;
    const shape = (w: number): Point[] => [a,
  ...
  const crescent = (cx: number, rx: number, ry: number, start: number, span: number,
    thickness: number, ink: number): void => {
    ...
    for (let i = 9; i < 19; i++) line(outer[i]!, outer[i + 1]!, shade(white, fade * 0.8));
  };

  const index = cast.formIndex ?? (cast.full ? SWORD_FORMS[art].length - 1 : 0);
  // 弯曲的带状笔触，和实体飞剑、月牙轮廓分别建模；不能再共用一个大三角。
  const ribbon = (points: Point[], width: number, ink: number): void => {
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      left.push([c[0] + nx * w, c[1] + ny * w]); right.unshift([c[0] - nx * w * 0.45, c[1] - ny * w * 0.45]);
    }
    fill([...left, ...right], shade(ink, fade));
    for (let i = 6; i < points.length - 3; i++) line(points[i]!, points[i + 1]!, shade(ink === 0x526875 ? 0x819398 : white, fade * 0.7));
  };
  const curve = (fn: (u: number) => Point, width: number, ink: number): void =>
    ribbon(Array.from({ length: 33 }, (_, i) => fn(i / 32)), width, ink);
  const seal = (cx: number, cy: number, r: number, ink: number): void => {
    const pts: Point[] = Array.from({ length: 13 }, (_, i) => [cx + Math.cos(i * Math.PI / 6) * r, cy + Math.sin(i * Math.PI / 6) * r]);
    fill(pts, shade(ink, fade));
  };
  // 剑身带护手；指劲则使用连续光波。两者不能只是换色的三角形。
  const sword = (cx: number, cy: number, angle: number, length: number): void => {
    ...
    switch (form.shape) {
      case 'thrust': {
        ...
        if (index === 1) {
          ...
          const cx = tip * 0.48;
          for (const s of [-1, 1]) line([cx, 0], [cx + s * h * 0.12, -h * 0.17], shade(color, fade * 0.75));
        } else {
      ...
      case 'sweep':
        crescent(form.radial ? 0 : reach * 0.35, reach * (form.radial ? 1 : 0.65), h * 0.35,
          -Math.PI + t * Math.PI * (index % 2 ? -1 : 1), Math.PI * 1.25, h * 0.14, color);
        break;
      case 'ring':
  ...
  if (art === 'dugu') {
    ...
    if (index === 0 || index === 2) {
      ...
      if (index === 2) for (const s of [-1, 1]) line([tip * 0.35, s * h * 0.13], [tip, s * h * 0.04], shade(color, fade * 0.5));
    } else if (index === 1) {
      // 商阳点射：两个短促脉冲前后追逐。
      for (let i = 0; i < 2; i++) {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        if (u > 0.65) curve(v => [cx + (v - 0.5) * h * 0.65, h * (0.46 - Math.sin(v * Math.PI) * 0.07)], h * 0.016, color);
      }
    }
  } else if (art === 'taiji') {
    ...
    switch (index) {
      case 0:
        ...
        break;
      case 1:
        for (const s of [-1, 1]) curve(u => [reach * u * 0.95, s * h * (Math.sin(u * Math.PI * 0.8) * (0.45 + t * 0.18))], h * 0.07, s > 0 ? yin : yang);
        break;
      case 2:
        ...
        break;
      case 3:
        for (const s of [-1, 1]) curve(u => [s * reach * 0.42 + Math.cos(u * Math.PI * 2 + s * motion) * reach * 0.38 * (1 - u * 0.8),
      ...
      case 4:
        ...
        break;
      case 5:
        for (const s of [-1, 1]) curve(u => [s * reach * (0.85 - clamp(t * 1.1) * 0.5) * Math.sin(u * Math.PI),
          h * (u - 0.5) * 1.1], h * 0.11, s > 0 ? yang : yin);
        break;
      case 6:
        for (let i = 0; i < 3; i++) curve(u => [(i - 1) * reach * 0.65 + Math.sin(u * Math.PI * 1.4 + motion) * reach * 0.18,
    ...
    const path = (u: number): Point => [reverse ? Math.sin(u * Math.PI) * tip : u * tip,
    ...
    if (index === 7) curve(u => [reach * (0.2 + u * 0.7), -h * 0.65 + Math.sin(u * Math.PI) * h * 0.5], h * 0.025, white);
  } else if (art === 'wanjian') {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (index === 0 || index === 6) {
        cx = lane * reach * 1.6; cy = -h * (0.12 + Math.cos(lane * Math.PI) * (index === 6 ? 0.55 : 0.3)); angle = -Math.PI / 2;
      } else if (index === 1 || index === 4) {
        cx = Math.cos(a) * reach * (index === 4 ? 0.9 - t * 0.55 : 0.72); cy = Math.sin(a) * h * 0.4; angle = a + Math.PI / 2;
      } else if (index === 5 || index === 8) {
        const fall = clamp((t - (i % 3) * 0.08) * 1.8);
        cx = lane * reach * 1.6; cy = -h * 0.55 + fall * h; angle = Math.PI / 2;
        if (index === 8) cx *= 1 - t * 0.35;
      } else {
        cx = reach * (0.15 + t * 0.75) - Math.abs(lane) * h;
        cy = lane * h * (index === 3 ? 1.2 - t : index === 7 ? 0.45 : 1.1); angle = index === 3 ? -lane * 0.4 : lane * 0.15;
      }
      sword(cx, cy, angle, h * (index === 7 ? 0.85 : 0.62));
      if (index === 0 || index === 6) seal(cx, h * 0.48, h * 0.025, color);
    }
  } else if (art === 'getsuga') {
    ...
    if (index === 5) crescent(cx - h * 0.55, rx * 0.7, ry * 0.7, -1.8, 3, h * 0.16, color);
    if (index === 3 || index === 6) for (const s of [-1, 1]) curve(u => [cx - h * (0.8 - u * 0.6), s * h * (0.1 + u * 0.28)], h * 0.045, 0x4963a2);
  } else {
    // 日轮：橙红火舌、金色火芯、离散余烬，不借用月牙或普通剑阵。
    const path = (u: number): Point => {
      if (index === 5) return [u * reach, Math.sin(u * Math.PI * 2 - t * 4) * h * 0.1];
      if (index === 7) return [u * reach, h * (-0.65 + u * 1.05)];
      if (index === 1) return [reach * (0.2 + u * 0.65), h * (0.36 - Math.sin(u * Math.PI * 0.8) * 1.05)];
      if (index === 6) return [reach * (0.3 + Math.cos(u * Math.PI * 1.8 + t) * 0.4), h * Math.sin(u * Math.PI * 1.8 + t) * 0.6];
    ...
    curve(path, h * 0.11, 0xe85536); curve(path, h * 0.038, 0xffcb75);
    for (let i = 0; i < 6; i++) {
      ...
      if (i % 2 === 0) seal(a[0] - h * t * 0.2, a[1] - h * (0.18 + t * 0.12), h * 0.022, 0xffd281);
    }
  }
}

/** 红线预告落点，白红地刺爆发；生命刻度与受击裂光三档共用。 */
export function paintBossPressure(p: ArtPen, w: World, armorOutline = true): void {
  const h = w.fh;
  // 各终端渲染档共用金色轮廓，不依赖文字能否放进两行 HUD。
  for (const f of armorOutline ? [w.player, ...w.enemies] : []) {
    if (f === w.player ? w.respawn > 0 || (f.armorT ?? 0) <= 0 : !bossArmored(f)) continue;
    for (const s of fighterSegments(f)) {
      if (s.part === 'head') p.circle(s.x0, s.y0, s.r, 0xffd66b);
      else p.line(s.x0, s.y0, s.x1, s.y1, 0xffd66b);
    }
    if (f === w.player) {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        p.line(cx + f.face * f.h * 0.15 * bulge0, y0,
          cx + f.face * f.h * 0.15 * bulge1, y1, 0xffd66b);
      }
    }
  }
  for (const e of w.enemies) if (e.tag === 'boss' || e.duelist) {
    if (e.enemyCast) paintSwordArt({ ...p, line: (x0, y0, x1, y1, ink) => {
    ...
    for (let i = 0; i < cells; i++) p.line(e.x - h * 0.7 + i * unit, e.y - e.h * 1.14,
      e.x - h * 0.7 + (i + 0.7) * unit, e.y - e.h * 1.14, i < filled ? 0xff785c : 0x453b45);
    if (e.hurt > 0.16) for (let i = 0; i < 6; i++) {
      const a = i / 6 * Math.PI * 2, r = h * (0.55 + (0.3 - e.hurt) * 3);
      p.line(e.x + Math.cos(a) * r, e.y - h * 0.5 + Math.sin(a) * r * 0.6,
        e.x + Math.cos(a) * (r + h * 0.3), e.y - h * 0.5 + Math.sin(a) * (r + h * 0.3) * 0.6, 0xfff0c7);
    }
    if (e.windup >= 0 || (e.followupT ?? 0) > 0) {
      if (e.duelist) {
        ...
        if (form.shape === 'rain') for (let i = 0; i < 5; i++) {
          const cx = e.x + (i / 4 * 1.8 - 0.9) * reach;
          p.line(cx, w.ground - h * 0.12, cx, w.ground - 0.6, 0xffc38b);
        }
        else p.line(e.x, e.y - h * 0.55, e.x + e.face * h * 0.8, e.y - h * 0.55, 0xffc38b);
      } else if (e.quakeX !== undefined) for (const offset of bossQuakeOffsets(e)) {
        ...
        p.line(Math.max(0, Math.min(w.w, start)), w.ground - 0.6,
          Math.max(0, Math.min(w.w, end)), w.ground - 0.6, color);
        if (e.bossKind === 'mantis' && bossAbilities(e).doubleStrike) {
          const x = e.x + e.face * reach * 0.55;
          for (const side of [-1, 1]) p.line(x - h * 0.15, e.y - h * (0.5 + side * 0.15),
            x + h * 0.15, e.y - h * (0.5 - side * 0.15), color);
        }
      }
      else {
  ...
  for (const hazard of w.hazards) {
    ...
    if (timer <= 0) for (const s of [-1, 0, 1]) {
