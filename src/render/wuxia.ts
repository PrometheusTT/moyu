...
import { SWORD_ARTS, currentSwordForm, artDuration } from '../core/martial.ts';
import type { SceneTheme } from './theme.ts';
import { bossPhase, bossQuakeOffsets, type World } from '../core/world.ts';
...
export function paintSwordArt(p: ArtPen, cast: SwordCast | null, h: number): void {
  ...
  const line = (a: number, b: number, c: number, d: number, ink: number = color): void =>
    p.line(x + face * a, y + b, x + face * c, y + d, ink);
  const arc = (cx: number, rx: number, ry: number, start: number, span: number, ink: number = color): void => {
    for (let i = 0; i < 24; i++) {
  ...
  if (art === 'dugu') {
    ...
    if (power === 2) for (let i = 0; i < 8; i++) {
      const a = i / 8 * Math.PI * 2;
      line(tip + Math.cos(a) * h * 0.1, Math.sin(a) * h * 0.1,
        tip + Math.cos(a) * h * 0.34, Math.sin(a) * h * 0.34, 0xfff0c7);
    }
  } else if (art === 'liumai') {
    ...
    for (let side = 0; side < 2; side++) {
      ...
      if (power > 0) arc(0, r * 0.78, h * 0.25, a + 0.15, Math.PI * 0.65);
    }
    // 奥义：外围再加一圈四段旋转罡气，太极图外有剑阵。
    if (power === 2) for (let k = 0; k < 4; k++) {
    ...
    for (let i = 1; i <= power + 1; i++)
      line(start + h * 0.15, -h * 0.8 + start / reach * h - i * h * 0.045,
        end * 0.8, -h * 0.8 + end / reach * h * 0.8 - i * h * 0.045);
    // 奥义：主剑光身后拖两道平行的残影，像同一剑被时光重放了两次。
    if (power === 2) for (let k = 1; k <= 2; k++) {
      const d = k * h * 0.32;
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (power === 2 && fall > 0.92) line(cx - h * 0.14, cy, cx + h * 0.14, cy, 0xfff0c7);
    }
  } else if (art === 'getsuga') {
    ...
    if (power === 2) arc(cx * 1.3, rx * 1.55, h * 0.78, -1.05, 2.1, 0xd7edff);
  } else {
    ...
    if (power === 2) arc(0, reach * grow * 1.14, h * 0.62, start - 0.06, span * 1.04, 0xffe9c8);
...
export function paintBossPressure(p: ArtPen, w: World): void {
  const h = w.fh;
  for (const e of w.enemies) if (e.tag === 'boss') {
    ...
    if (e.windup >= 0) {
      if (e.quakeX !== undefined) for (const offset of bossQuakeOffsets(e)) {
        const cx = Math.max(0, Math.min(w.w, e.quakeX + offset * h * 1.5));
        p.line(cx - h * 0.48, w.ground - 0.6, cx + h * 0.48, w.ground - 0.6, 0xff674b);
      }
      else p.line(e.x, w.ground - 0.6, e.x + e.face * h * (bossPhase(e.hp, e.maxHp) > 1 ? 3 : 1.4), w.ground - 0.6, 0xff674b);
    }
  }
