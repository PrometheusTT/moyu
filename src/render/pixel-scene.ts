import { heroHat, type Pose, type Seg } from '../core/stick.ts';
import { bossPhase, type Fighter, type Piece, type World } from '../core/world.ts';
...
export function paintPixelWorld(c: PixelCanvas, world: World, context: PixelRenderContext,
  ...
  if (scene !== undefined) {
    if (scene.landmark) paintLandmark(pen, world.w, world.ground, scene);
    else paintPixelProps(c, world, scene, x, y, scale, ink, palette.key, skyBase);
  }
  ...
  const bossColor = (hp: number, maxHp = 5): number => {
  ...
  const drawBody = (body: Fighter, hero: boolean, base: number): void => {
  ...
  const draw = (fighter: Fighter, hero: boolean): void => {
    ...
    if (body.tag === 'boss' && body.dashT > 0) {
      for (const back of [0.45, 0.9]) drawBody({ ...body, x: body.x - body.face * world.fh * back }, false, mix(base, palette.key, 0.55));
    }
    ...
    if (body.tag === 'boss' && !body.duelist) {
      ...
      for (const ox of [-s, 0, s]) c.circle(x(body.x + ox), y(topY - s), Math.max(0.6, s * 0.42 * scale), ink(base));
    }
  };
