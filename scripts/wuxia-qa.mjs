...
const width = 720, rowHeight = 180, rows = previewArt ? SWORD_FORMS[previewArt].length + 1 : 17;
...
for (let row = 0; row < rows - 1; row++) {
  ...
  world.resize(180, 44); world.biome = row < 6 ? row : 2;
  ...
  if (!previewArt && row >= 6 && row <= 12) {
    world.swordCast = { art: ['dugu', 'liumai', 'taiji', 'feixian', 'wanjian', 'getsuga', 'hinokami'][row - 6], x: 65, y: 27,
      face: 1, age: 0.3, pulse: 1, level: 1 };
    world.player.atk = 0.16;
  }
  if (!previewArt && row >= 13) {
  ...
  if (previewArt) world.swordCast = { art: previewArt, x: 65, y: 27, face: 1, full: true,
    age: row === 0 ? 0.08 : 0.7, pulse: row, level: 1 };
  const t = new GraphicsTarget(80, 6, width / 80, rowHeight / 6);
  paintPixelWorld(new NativePixelCanvas(t), world, { view: 'expanded', interpolation: 1, theme: 'dark' }, undefined, false, sceneForChapter(row < 6 ? row + 1 : 1));
  copyRow(t, row);
}
const game = BUILTIN_GAMES[0].create({ seed: 31, random: () => 0.5 });
const logical = new LogicalCanvas(80, 20);
game.renderExpanded(logical); copyRow(logical, rows - 1, 9);
