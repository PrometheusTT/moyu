...
- 七套剑法按剑气档位出招：起手 / 60气强化 / 100气奥义一记统一收招（约1.2秒，不再轮播所有分式），按剑法累计熟练度。
- 储气上限从100提升至300；奥义只扣100，普通式保留零头，旧存档兼容。
- 战场占右侧约三分之二并随终端加宽；左侧常驻 HUD 只留战斗摘要与当前剑招，完整剑谱留在 ? 帮助页。
- 主角新增斗笠剪影与全档位红围巾；剑招共用出鞘闪光，奥义带落地气浪。
- 修复宽屏下移速随战场宽度失控放大：移速只跟身高走，宽战场是更长的距离而不是更快的人。
- 展开/图形档战场按纵横比真实加宽（封顶360世界单位），不再是横向拉伸同一幅画。
- 奥义出手配顿帧+爆闪+震屏节拍，演出期间每个伤害脉冲亮一记白环；七套奥义各加专属加强层（星芒/十字花/罡气环/残影/接地刻痕/回声月牙/白热火舌）。
...
- 普通攻击缩短为 320ms，命中停顿缓存输入，攻击时可走位，命中后可取消收招。
