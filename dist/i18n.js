import { SWORD_ARTS, SWORD_FORMS, FULL_ART_NAMES, ART_IDS } from "./core/martial.js";
const SYSTEM_LOCALE = Intl.DateTimeFormat().resolvedOptions().locale;
export function resolveGameLanguage(env = process.env, systemLocale) {
...
export const isEnglish = () => resolveGameLanguage() === 'en';
const ART_EN = {
...
const CHAPTER_EN = ['Rain in the Bamboo Sea', 'Bridge Beneath the Crescent Moon', 'Demons at the Gate',
    ...
    'Night Crossing', 'The Snow Mountain Lair', 'Dawn at Heaven\'s Gate'];
const STORY_EN = ['The leaves fall. A sword awakens.', 'Cold water remembers the road behind.',
    ...
    'The last cry fades into the mountain wind.', 'At first light, another road begins.'];
export function chapterName(chapter, chinese) {
    return isEnglish() ? CHAPTER_EN[(chapter - 1) % CHAPTER_EN.length] ?? chinese : chinese;
}
export function chapterStory(chapter, chinese) {
...
export function uiName(name) { return isEnglish() ? UI_NAMES[name] ?? name : name; }
const NAMED_EN = new Map();
...
export function englishBattleNotice(notice) {
    const growth = /^修为(\d+)重 · 体魄(\d+) · 技能周转提升$/.exec(notice);
    if (growth)
        return `Cultivation ${growth[1]} · Max HP ${growth[2]} · Faster skills`;
    const cooldown = /^护体未就绪 (\d+)秒$/.exec(notice);
    if (cooldown)
        return `Guard ready in ${cooldown[1]}s`;
    const locked = /^(.+)未悟 · 阅历(\d+)\/(\d+)$/.exec(notice);
    if (locked)
        return `${NAMED_EN.get(locked[1]) ?? 'Art'} locked · Insight ${locked[2]}/${locked[3]}`;
    const qi = /^剑气不足 (\d+)\/(\d+)$/.exec(notice);
    if (qi)
        return `Not enough qi ${qi[1]}/${qi[2]}`;
    if (notice === '护体罡气 · 霸体')
        return 'Iron Guard · Control immunity';
    if (notice === '铮！拼剑 · 剑招 / 绕背破守')
        return 'Blades clash! Use an art or attack from behind';
    if (notice === '铮！截剑成功 +6气')
        return 'Parry! +6 qi';
    const encounter = /^(剑宗临阵|剑客问剑) · (青锋|玄衣)$/.exec(notice);
    if (encounter)
        return `${encounter[1] === '剑宗临阵' ? 'Sword master appears' : 'Duelist challenges you'} · ${uiName(encounter[2])}`;
