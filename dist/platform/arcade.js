...
import { World, bossPhase, BOSS_NAMES, bossAbilities } from "../core/world.js";
...
import { MAX_QI, BOSS_HIT_QI, SWORD_ARTS, SWORD_FORMS, FULL_ART_NAMES, parseFormProgress, selectSwordForm, currentSwordForm, ART_IDS, isSecretArt, artLevel, freshCultivation, parseCultivation, playerGrowth } from "../core/martial.js";
 ...
 */
function paintClearBurst(c, pulse) {
    if (pulse <= 0)
        return;
    ...
    if (c.height < 12) {
        ...
        for (const s of [-1, 1]) {
            ...
            if (bx >= 0 && bx < c.width)
                c.line(bx, 0, bx, c.height - 1, color);
        }
        if (bright > 0.2)
            for (let x = 0; x < c.width; x++)
                c.pixel(x, 0, mixRgb(AMBER, INK, bright * 0.5));
        return;
    }
    const cx = c.width / 2, cy = c.height * 0.46;
    const maxR = Math.max(c.width, c.height) * 0.62;
    for (const [rf, wf] of [[1, 1], [0.62, 0.6]]) {
    ...
    if (bright > 0.15) {
        ...
        for (let i = 0; i < embers; i++) {
            ...
            if (ey >= 0)
                c.pixel(Math.round(ex), Math.round(ey), mixRgb(AMBER, INK, bright));
        }
    }
}
...
class InputLatch {
    ...
    feed(bytes, now, armorEnabled = false) {
        let played = false;
...
class StickGame {
    ...
    update(dt, input) {
        ...
        if (!settledBefore && this.director.result !== null)
            this.clearPulse = 1;
        else
            this.clearPulse = Math.max(0, this.clearPulse - dt / 1.1);
    ...
    renderMicro(c) {
        ...
        paintLandmark(backdrop, c.width * this.world.ground / 7, this.world.ground, { ...scene, groundTint: scene.skyTint, foeTint: scene.skyTint });
        ...
        if (this.world.respawn <= 0)
            drawMicroFighter(c, this.world.player, playerX, (this.world.player.armorT ?? 0) > 0 ? 0xffd66b : INK, AMBER, Math.min(1, (this.world.ground - this.world.player.y) / 10));
        else {
    ...
    restore(state) {
        ...
        const restoreArts = () => {
            ...
            this.world.selectedArt = ART_IDS.includes(s.selectedArt) ? s.selectedArt : 'dugu';
            this.intermission = 0;
            this.previous.clear();
            this.scratch.clear();
        };
