...
function readManifest(dir) {
    return validateManifest(JSON.parse(fs.readFileSync(path.join(dir, MANIFEST), 'utf8')));
}
function installed() {
    let names;
    try {
    ...
    for (const name of names.sort())
        try {
            ...
            if (fs.statSync(dir).isDirectory())
                out.push({ dir, manifest: readManifest(dir) });
        }
...
export async function loadGameModules() {
    ...
    for (const item of installed())
        try {
            if (modules.some((m) => m.manifest.id === item.manifest.id))
                continue;
            const entry = path.resolve(item.dir, item.manifest.entry);
            if (!entry.startsWith(`${path.resolve(item.dir)}${path.sep}`))
                continue;
            const loaded = await import(__rewriteRelativeImportExtension(pathToFileURL(entry).href));
...
export async function cmdGames(args) {
