    ...
    render(game, target, top, cols, rows) {
        ...
        if (mode !== this.lastMode) {
            for (let row = 0; row < rows; row++)
                clear += `\x1b[m\x1b[${top + row};1H${' '.repeat(width)}`;
            target.invalidate();
            this.lastPanel = '';
            this.lastMode = mode;
        }
        if (help)
            ...
            game.render(target, rows <= 2 ? 'micro' : 'standard');
        ...
        const panel = available === 0 ? '' : lines.map((line, i) => `\x1b[m\x1b[${top + i};${left}H${fitRow(line.replace(/[\x00-\x1f\x7f-\x9f]/g, ''), '', available)}`).join('');
