...
export class LogicalCanvas {
    ...
    pixel(x, y, color) { this.setPixel(x, y, color); }
    setPixel(x, y, color) {
        x = Math.floor(x);
        y = Math.floor(y);
    ...
    rect(x, y, w, h, color) { this.fillRect(x, y, w, h, color); }
    fillRect(x, y, w, h, color) {
        const x0 = Math.max(0, Math.floor(x)), y0 = Math.max(0, Math.floor(y));
        const x1 = Math.min(this.width, Math.ceil(x + w)), y1 = Math.min(this.height, Math.ceil(y + h));
        for (let yy = y0; yy < y1; yy++)
            this.pixels.fill(color, yy * this.width + x0, yy * this.width + x1);
    }
    ...
    resize(_cols, _rows) { }
    invalidate() { }
    encode(_screenTop) { return ''; }
    disposeSeq() { return ''; }
    blit(target, matte = 0x090a0e) {
        target.fill(matte);
