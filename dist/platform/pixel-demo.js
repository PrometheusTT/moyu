...
export async function cmdPixelDemo() {
    ...
    let paused = false, legacy = false, expanded = false, theme = process.env.MOYU_THEME === 'light' ? 'light' : 'dark';
    let timer, done = false, blocked = false, dirty = true;
    let last = performance.now(), accumulator = 0;
    return new Promise(resolve => {
        const finish = () => { if (done)
            return; done = true; teardown.run(); resolve(0); };
        const onData = (bytes) => {
            if (bytes.includes(3) || bytes.includes(27) || bytes.includes(113)) {
                finish();
                return;
            }
        ...
        const onResize = () => {
            if (process.stdout.columns < 42 || process.stdout.rows < 12) {
                finish();
                return;
            }
            dirty = true;
        ...
        const draw = () => {
            if (done)
                return;
            const now = performance.now(), elapsed = Math.min(0.25, (now - last) / 1000);
            last = now;
            if (!paused) {
                accumulator += elapsed;
            ...
            dirty = false;
            if (out)
                blocked = !process.stdout.write(out);
        };
        draw();
        timer = setInterval(draw, 1000 / caps.fps);
    });
}
