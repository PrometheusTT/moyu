 ...
 * M1 会用真的世界状态替换掉这个文件。
 */
import { rgb } from "../render/canvas.js";
/** 天空的 4 段渐变 + 地面 + 精灵。限定调色板是硬性要求，不是省事。 */
const SKY = [rgb(12, 14, 30), rgb(20, 22, 46), rgb(30, 30, 62), rgb(44, 38, 74)];
 ...
 */
export function paintScene(cv, t) {
    const w = cv.pixelW;
    const h = cv.pixelH;
    if (w < 1 || h < 1)
        return;
 ...
 * 用来量上界，不是用来当场景。
 */
export function paintStress(cv, t) {
    const w = cv.pixelW;
    const h = cv.pixelH;
    if (w < 1 || h < 1)
        return;
...
function paintBackground(cv, w, h) {
