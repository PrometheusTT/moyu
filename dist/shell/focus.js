    ...
    if (next !== 0x5b) {
        if (next === 0x5d || next === 0x50 || next === 0x58 || next === 0x5e || next === 0x5f) {
            for (let j = at + 2; j < buf.length; j++) {
                if (buf[j] === 0x07)
                    return { end: j + 1, key: null, reply: true };
                if (buf[j] === 0x1b && buf[j + 1] === 0x5c)
                    return { end: j + 2, key: null, reply: true };
            }
        }
        if (next === 0x4f && buf[at + 2] !== undefined)
            return { end: at + 3, key: null, reply: false };
...
export class InputRouter {
    ...
    route(chunk) {
        ...
        while (i < buf.length) {
            ...
            if (b === 0x1b) {
                ...
                if (scan !== null) {
                    ...
                    if (seq === '\x1b[200~' || seq === '\x1b[201~') {
                        flush(i);
                        actions.push({ kind: 'forward', bytes });
                    ...
                    if (seq === '\x1b[24~') {
                        flush(i);
                        actions.push({ kind: 'toggle-focus' });
                        i = scan.end;
                        continue;
                    }
            ...
            if (b === 0x00) {
                flush(i);
                actions.push({ kind: 'forward', bytes: Uint8Array.of(b) });
                i++;
                continue;
            }
