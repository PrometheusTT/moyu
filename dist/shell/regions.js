 ...
 * ## 尺寸
 *
 * 候场恒定一行，但只绘制一个非动画状态点；默认游戏条恒定两行，并始终给 CLI 留至少 10 行。
...
/** 手动调整游戏区行数，返回夹取后的合法值。 */
export function adjustGameRows(rows, current, delta) {
    return clampRows(rows, current + delta);
}
/** 断言不变式。开发期调用，生产期也留着 —— 它坏了整个方案就坏了。 */
export function assertLayout(l) {
    if (l.innerTop !== 1)
        throw new Error(`不变式被破坏：innerTop 必须是 1，得到 ${l.innerTop}`);
    if (l.gameTop !== l.innerRows + 1)
        throw new Error(`几何不自洽：gameTop=${l.gameTop} innerRows=${l.innerRows}`);
    if (l.innerRows + l.gameRows !== l.rows)
        throw new Error(`行数不守恒：${l.innerRows}+${l.gameRows}≠${l.rows}`);
    if (l.innerRows < MIN_INNER_ROWS)
        throw new Error(`内层行数不足：${l.innerRows}`);
    if (l.gameRows < MIN_GAME_ROWS)
        throw new Error(`游戏区行数不足：${l.gameRows}`);
    if (l.gameRows > MAX_GAME_ROWS)
        throw new Error(`游戏区行数超上限：${l.gameRows}`);
    // 场地必须给 HUD 留出至少一列，而且**不能占满整行** —— 写到最后一列会置上终端的
    // 延迟换行标志，下一个可打印字符就会把屏幕滚上去一行，游戏条从此错位。
    if (l.fieldCols >= l.cols)
        throw new Error(`场地占满了整行：fieldCols=${l.fieldCols} cols=${l.cols}`);
}
/** 真实终端的滚动区序列 —— 把内层的换行限制在上半屏。 */
export function scrollRegionSeq(l) {
    return `\x1b[1;${l.innerRows}r`;
}
/** 撤掉滚动区（让屏 / 退出时用）。 */
export function fullScrollRegionSeq() {
    return '\x1b[r';
}
