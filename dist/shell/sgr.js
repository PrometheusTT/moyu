 ...
 */
export function applySgr(s, params) {
    if (params.length === 0) {
        resetSgr(s);
        return;
    }
...
function readExtColor(params, at) {
...
export function emitSgr(s) {
    ...
    if (s.underline === 1)
        p.push(4);
    else if (s.underline === 2)
        p.push(21);
    else if (s.underline >= 3)
        p.push(`4:${s.underline}`);
    if (s.blink)
        p.push(5);
    if (s.reverse)
        p.push(7);
