...
function sgrFg(c) { return `\x1b[38;2;${(c >> 16) & 255};${(c >> 8) & 255};${c & 255}m`; }
function sgrBg(c) { return `\x1b[48;2;${(c >> 16) & 255};${(c >> 8) & 255};${c & 255}m`; }
const DEFAULT_SGR = -2;
...
export class BrailleTarget {
    ...
    constructor(cols, rows, options = {}) {
        ...
        this.bg = new Uint32Array(0);
        this.prevMask = new Uint8Array(0);
        this.prevFg = new Uint32Array(0);
        this.prevBg = new Uint32Array(0);
        this.alloc();
    }
    get pixelW() { return this.cols * 2; }
    get pixelH() { return this.rows * 4; }
    alloc() {
        ...
        this.prevMask = new Uint8Array(n);
    ...
    fill(color) { this.frameMatte = color; this.pixels.fill(color); }
    fillRect(x, y, w, h, color) {
        const x0 = Math.max(0, x), y0 = Math.max(0, y);
        const x1 = Math.min(this.pixelW, x + w), y1 = Math.min(this.pixelH, y + h);
        for (let yy = y0; yy < y1; yy++)
            this.pixels.fill(color, yy * this.pixelW + x0, yy * this.pixelW + x1);
    }
    setPixel(x, y, color) {
        if (x < 0 || x >= this.pixelW || y < 0 || y >= this.pixelH)
            return;
    ...
    invalidate() { this.dirtyAll = true; }
    setGlyphStyle(style) {
        if (this.glyphStyle !== style) {
            this.glyphStyle = style;
            this.invalidate();
        }
    }
    disposeSeq() { return ''; }
    quantizeCell(col, row, at) {
        const colors = [], counts = [];
        for (let x = 0; x < 2; x++)
            for (let y = 0; y < 4; y++) {
        ...
        if (fg !== bg) {
            ...
            for (let x = 0; x < 2; x++)
                for (let y = 0; y < 4; y++, n++) {
                    const c = this.pixels[(row * 4 + y) * this.pixelW + col * 2 + x];
                    if (distance(c, fg) <= distance(c, bg))
                        mask |= DOT[n];
                }
        }
        this.bg[at] = bg;
    ...
    encode(screenTop, screenLeft = 1) {
        const n = this.cols * this.rows;
        for (let i = 0; i < n; i++)
            this.quantizeCell(i % this.cols, Math.floor(i / this.cols), i);
        const out = [];
        let curFg = -1, curBg = -1;
        ...
        for (let row = 0; row < this.rows; row++) {
            ...
            while (col < this.cols) {
                ...
                out.push(`\x1b[${screenTop + row};${start + screenLeft}H`);
                for (let x = start; x < end; x++) {
                    ...
                    if (bgKey !== curBg) {
                        out.push(bgKey === DEFAULT_SGR ? '\x1b[49m' : sgrBg(eb));
                        curBg = bgKey;
                    }
                    if (m === 0) {
                        out.push(' ');
                    ...
                    if (fgKey !== curFg) {
                        out.push(fgKey === DEFAULT_SGR ? '\x1b[39m' : sgrFg(ef));
                        curFg = fgKey;
                    }
        ...
        this.lastBytes = Buffer.byteLength(encoded);
        return encoded;
    }
}
