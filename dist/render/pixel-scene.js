...
export function snapshotFighters(world) {
...
export function paintPixelWorld(c, world, context, previous) {
    ...
    const draw = (f, hero) => {
        ...
        for (const s of segs) {
            ...
            const radius = s.part === 'head' ? s.r * scale : Math.max(0.55, body.h * scale / (s.part === 'torso' ? 28 : s.part === 'blade' ? 48 : 38));
            ...
            if (hero && s.part === 'torso') {
                ...
                c.stroke(x(s.x1), y(s.y1), x(s.x1 - f.face * f.h * 0.22), y(s.y1 + f.h * 0.08), Math.max(0.55, body.h * scale / 45), palette.accent);
            }
        }
    ...
    if (world.respawn <= 0)
        ...
        c.stroke(x(player.x - 4), y(world.ground - 1), x(player.x + 4), y(world.ground - 1), Math.max(0.6, scale), palette.accent);
}
