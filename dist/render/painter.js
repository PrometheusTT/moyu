...
export function painterFor(t, vh) {
...
export function clear(p, color) {
    p.t.fill(color);
}
/** 实心矩形，虚拟坐标**半开区间** `[x0,x1) × [y0,y1)`。 */
export function rect(p, x0, y0, x1, y1, color) {
...
/** 实心圆（头）。 */
export function disc(p, cx, cy, r, color, extra = 0) {
 ...
 */
export function arcStroke(p, cx, cy, r, a0, a1, w, color) {
    const X = (cx + 0.5) * p.k;
    const Y = (cy + 0.5) * p.k;
    const R = r * p.k;
    if (!Number.isFinite(X + Y + R) || R < 0.5)
        return;
