 ...
 */
export const DEFAULT_CELL = { w: 16, h: 34 };
/**
 ...
 */
export function parseProbe(s, cols, rows) {
    let graphics = false;
    let cellW;
    let cellH;
    let done = false;
    ...
    while (i < s.length) {
        ...
        if (s.startsWith('\x1b_G', i)) {
            ...
            if (body.includes(`i=${PROBE_ID}`) && body.includes(';OK'))
                graphics = true;
            i = end;
            continue;
        }
 ...
 */
export function knownGraphicsTerm(env) {
    const term = (env.TERM ?? '').toLowerCase();
    if (term.includes('kitty'))
        return 'kitty';
    if (term.includes('ghostty'))
        return 'Ghostty';
    if (term.includes('wezterm'))
        return 'WezTerm';
    ...
    if (env.KITTY_WINDOW_ID !== undefined && env.KITTY_WINDOW_ID !== '')
        return 'kitty';
    if (env.GHOSTTY_RESOURCES_DIR !== undefined && env.GHOSTTY_RESOURCES_DIR !== '')
        return 'Ghostty';
    if (env.WEZTERM_PANE !== undefined && env.WEZTERM_PANE !== '')
        return 'WezTerm';
    return null;
}
function parseCellEnv(v) {
    if (v === undefined || v === '')
        return null;
