 ...
 */
import { rgb } from "./canvas.js";
import { arcStroke, disc, dot, rect, stripPainter, stroke } from "./painter.js";
import { bodyOf, bossPhase } from "../core/world.js";
...
const SKY = [rgb(12, 13, 18), rgb(17, 18, 25), rgb(23, 24, 33), rgb(30, 31, 42)];
...
export function paintWorld(p, w, scene) {
    ...
    for (const q of w.pieces)
        if (q.rest)
            paintPiece(p, w, q, dx, dy, tint(PIECE_DEAD, dim));
    for (const q of w.pieces)
        if (!q.rest)
    ...
    for (const e of w.enemies) {
        ...
        if (e.tag === 'boss' && !e.duelist)
            paintBossCrown(p, w, e, dx, dy, tint(c, dim));
    }
    if (w.respawn <= 0) {
        const f = w.player;
 ...
 */
function paintFighter(p, w, f, dx, dy, color, hero) {
    const body = bodyOf(f);
    ...
    for (const s of segs) {
        ...
        if (hero && s.part === 'torso') {
            stroke(p, s.x1 + dx, s.y1 + dy, s.x1 + dx - f.face * w.fh * 0.26, s.y1 + dy + w.fh * 0.1, body.h * R_LIMB * 0.9, ACCENT);
        }
    }
}
/** 一段。头是圆，其余是带圆头的线段（胶囊）—— 胶囊在肩肘髋处天然接得上，见 `painter.ts`。 */
function paint1(p, s, dx, dy, r, color, extra) {
    if (s.part === 'head')
        disc(p, s.x0 + dx, s.y0 + dy, r, color, extra);
    else
        stroke(p, s.x0 + dx, s.y0 + dy, s.x1 + dx, s.y1 + dy, r, color, extra);
}
/** 分部位笔宽。头的半径由 `segments()` 直接给（它按身高算过了）。 */
function radiusOf(s, h, flash) {
    if (s.part === 'head')
        return s.r;
    if (s.part === 'torso')
        return h * R_TORSO;
    if (s.part === 'blade')
        return h * R_BLADE * (flash ? 1.9 : 1);
    return h * R_LIMB;
}
...
function paintPiece(p, w, q, dx, dy, color) {
    ...
    if (key)
        stroke(p, q.x - cx + dx, q.y - cy + dy, q.x + cx + dx, q.y + cy + dy, r, KEY, KEYLINE);
 ...
 */
function paintSlash(p, w, s, dx, dy, dim) {
 ...
 */
function bossBaseColor(hp, maxHp = 5) {
