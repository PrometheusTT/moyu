import { segments } from "./stick.js";
/** 身体和死亡碎片共用同一轮廓：六足菱蛛、三角螳螂、方甲虫、八足蛛王。 */
export function fighterSegments(f) {
    if (f.kind === 'player' || f.tag === undefined)
        return segments(f);
    const out = [];
    const h = f.h, x = f.x, y = f.y;
    const line = (a, b, c, d, part = 'torso') => {
    ...
    if (!boss && f.species) {
        ...
        switch (f.species) {
            case 'mantis':
                poly([[-0.3, -0.25], [0, -0.85], [0.32, -0.3], [-0.3, -0.25]]);
                legs(2, 0.3);
                for (const s of [-1, 1])
                    poly([[s * 0.15, -0.55], [s * 0.65, -0.9], [s * 0.5, -0.35]]);
                break;
            case 'crab':
                poly([[-0.4, -0.5], [0.4, -0.5], [0.35, -0.2], [-0.35, -0.2], [-0.4, -0.5]]);
                legs(3, 0.18);
                for (const s of [-1, 1])
                    poly([[s * 0.35, -0.4], [s * 0.7, -0.7], [s * 0.85, -0.48], [s * 0.55, -0.55]]);
                break;
            case 'eel':
                for (let i = 0; i < 7; i++) {
                ...
                break;
            case 'scorpion':
                poly([[-0.35, -0.25], [-0.35, -0.5], [0.35, -0.5], [0.35, -0.25], [-0.35, -0.25]]);
                legs(3, 0.2);
                poly([[-0.3, -0.4], [-0.75, -0.65], [-0.65, -1.05], [-0.3, -1.1], [-0.2, -0.85]]);
                poly([[0.3, -0.4], [0.8, -0.6], [0.65, -0.25]]);
                break;
            case 'scarab':
                poly([[-0.4, -0.3], [-0.32, -0.7], [0.32, -0.7], [0.4, -0.3], [-0.4, -0.3]]);
                legs(3, 0.15);
                line(0, -0.7, 0, -0.3);
                poly([[0.3, -0.6], [0.6, -0.85], [0.5, -0.45]]);
                break;
            case 'wolf':
                poly([[-0.5, -0.5], [0.25, -0.55], [0.4, -0.9], [0.5, -0.65], [0.8, -0.5], [0.3, -0.3], [-0.5, -0.3], [-0.5, -0.5], [-0.8, -0.75]]);
                for (const s of [-1, 1])
                    for (const offset of [-0.1, 0.1])
                        poly([[s * 0.35, -0.35], [s * 0.45 + offset, -0.2], [s * 0.55 + offset + gait, 0]]);
                break;
            case 'crystal':
                for (const s of [-1, 0, 1])
                    poly([[s * 0.25 - 0.2, -0.3], [s * 0.4, s === 0 ? -1 : -0.75], [s * 0.25 + 0.2, -0.3]]);
                legs(3, 0.2);
                break;
            case 'bat':
                poly([[-0.12, -0.3], [0, -0.85], [0.12, -0.3], [-0.12, -0.3]]);
                for (const s of [-1, 1])
                    poly([[0, -0.6], [s * 0.8, -0.95 + gait * 2], [s * 0.6, -0.35], [s * 0.4, -0.5], [s * 0.2, -0.25], [0, -0.6]]);
                break;
            case 'idol':
                poly([[-0.35, -0.3], [-0.3, -0.85], [0.3, -0.85], [0.35, -0.3], [-0.35, -0.3]]);
                legs(3, 0.2);
                poly([[-0.3, -0.85], [-0.4, -1.05], [0, -0.9], [0.4, -1.05], [0.3, -0.85]]);
                line(-0.15, -0.48, 0.15, -0.48);
                break;
        }
        for (const eye of [-0.1, 0.1])
            out.push({ part: 'head', x0: x + eye * h, y0: y - h * 0.55, x1: x, y1: y, r: h * 0.045 });
        return out;
    }
