...
export class World {
    ...
    taskRngState = null;
    time = 0;
    constructor(seed = 0x5eed1234, options = {}) {
    ...
    stepSwordArt(dt) {
        ...
        for (let i = this.enemies.length - 1; i >= 0; i--) {
            ...
            if (e.tag === 'boss') {
                ...
                if (hits >= (cast.full ? 3 : form.qi >= 60 ? 2 : 1))
                    continue;
    ...
    spawnFormation(formation) {
        ...
        const sides = formation.kind === 'single' ? [formation.side]
        ...
        if (sides.length === 0 || sides.length > capacity)
            return false;
    ...
    spawnDuelist(side, chapter, boss = false) {
        if (this.enemies.length >= this.enemyLimit || this.enemies.some(e => e.duelist))
            return false;
    ...
    spawn(dt) {
        ...
        const side = this.rng.chance(0.5) ? 'left' : 'right';
    ...
    poseFor(f) {
        ...
        if (f.dashT > 0)
            return poseSlash(0.45);
        if (f.spinT > 0)
            return poseSlash(clamp(1 - f.spinT / SPIN_TIME, 0, 1));
        if (f.atk >= 0)
            return poseSlash(f.atk < 0.1 ? (f.h < 8 ? 0.05 : 0.18) : f.atk < 0.19 ? 0.38 : 0.85, f.atkKind ?? 'normal');
        if (f.windup >= 0)
