...
常见问题及恢复步骤见 [故障排查指南](./docs/troubleshooting.md)。如果仍无法解决，可以创建
[Bug report](https://github.com/PrometheusTT/moyu/issues/new?template=bug_report.yml)。

请附上 Moyu 版本或 commit、Node.js 版本、操作系统、终端、连接方式和最小复现步骤。分享诊断
输出前，请删除用户名、主机名、IP、本地路径、token、prompt 和私有仓库内容。

## 功能与 Cartridge 开发

- 功能建议：使用 [Feature request](https://github.com/PrometheusTT/moyu/issues/new?template=feature_request.yml)。
- Cartridge API：阅读 [Cartridge 开发指南](./docs/cartridge-api.md)。
- 贡献代码：阅读 [CONTRIBUTING.md](./CONTRIBUTING.md)。

## 不适合公开 Issue 的内容

- 安全漏洞：按照 [SECURITY.md](./SECURITY.md) 私密报告。
- 行为准则事件：按照 [CODE_OF_CONDUCT.md](./CODE_OF_CONDUCT.md) 私密报告。
- 包含凭据、完整环境变量、真实用户数据或私有 agent 会话的日志。

项目由社区维护，不提供保证响应时间的商业支持。
